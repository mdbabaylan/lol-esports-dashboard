...
const MyBetsPage = () => {
  ...
  const fetchBets = async () => {
    try {
      ...
      const winRate = overall.total_bets > 0 
        ? ((overall.wins / overall.total_bets) * 100).toFixed(1)
        : 0
      
      setStats({
        totalBets: overall.total_bets || 0,
        wins: overall.wins || 0,
        losses: overall.losses || 0,
        pending: overall.pending || 0,
        winRate: winRate,
        totalPnl: overall.total_pnl || 0,
        avgWin: overall.avg_win || 0,
        avgLoss: overall.avg_loss || 0,
        avgEdge: (overall.avg_edge * 100).toFixed(2) || 0,
      })
      
      setLeagueStats(statsData.byLeague || [])
    } catch (err) {
      console.error('Error fetching bets:', err)
      setError('Failed to load bets from database. Make sure the server is running.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const formatOdds = (odds) => {
    ...
    if (odds >= 0.5) {
      ...
      return `+${Math.round(((1 - odds) / odds) * 100)}`
    }
  }
  ...
  const getLeagueColor = (league) => {
    switch (league?.toUpperCase()) {
      case 'LPL': return '#ef4444' // Red for China
      case 'LCK': return '#3b82f6' // Blue for Korea
      case 'LEC': return '#22c55e' // Green for EU
      case 'LCS': return '#a855f7' // Purple for NA
          ...
          <Stack direction="row" alignItems="center" spacing={1.5}>
            ...
            <Box>
              ...
              <Typography variant="caption" color="text.secondary">
                LoL Esports Betting History (CSV Import)
              </Typography>
            </Box>
          </Stack>
          <Chip
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <StatCard
              title="Win Rate"
              value={`${stats.winRate}%`}
              subtitle={`${stats.wins} / ${stats.totalBets} correct`}
              icon={<EmojiEvents />}
              color="success"
          ...
          <Grid item xs={12} sm={6} md={3}>
            <StatCard
              ...
              subtitle={`Avg Edge: ${stats.avgEdge}%`}
          ...
          <>
            ...
            <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
              {leagueStats.map((league) => (
                <Chip
                  key={league.league}
                  label={`${league.league}: ${league.wins}W/${league.losses}L • ${formatCurrency(league.pnl)}`}
                  size="small"
                  sx={{
                    bgcolor: `${getLeagueColor(league.league)}20`,
                    color: getLeagueColor(league.league),
                    fontWeight: 600,
                    border: `1px solid ${getLeagueColor(league.league)}40`,
                  }}
                />
              ))}
            </Stack>
            ...
            <Table size="small">
              <TableHead>
                <TableRow sx={{ bgcolor: 'background.default' }}>
                  ...
                  <TableCell sx={{ fontWeight: 700 }} align="center">Market</TableCell>
                  <TableCell sx={{ fontWeight: 700 }} align="center">My Prob</TableCell>
                  <TableCell sx={{ fontWeight: 700 }} align="center">Edge</TableCell>
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align="center">
                          <Tooltip title={`Market odds: ${(bet.market_odds * 100).toFixed(1)}%`} arrow>
                            <Chip
                              label={formatPercent(bet.market_odds)}
                              size="small"
                              sx={{
                                fontWeight: 600,
                                fontSize: '0.7rem',
                        ...
                        <TableCell align="center">
                          <Chip
                            label={formatPercent(bet.my_prob)}
                            size="small"
                            sx={{
                              fontWeight: 600,
                              fontSize: '0.7rem',
                        ...
                        <TableCell align="center">
                          <Typography
                            variant="body2"
                            fontWeight={700}
                            color={bet.edge > 0 ? 'success.main' : bet.edge < 0 ? 'error.main' : 'text.secondary'}
                          >
                            {bet.edge > 0 ? '+' : ''}{formatPercent(bet.edge)}
                          </Typography>
                        </TableCell>
